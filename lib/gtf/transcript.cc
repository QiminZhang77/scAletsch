...
int transcript::write(ostream &fout) const
{
	...
	if(gene_type != "") fout<<"gene_biotype \""<<gene_type.c_str()<<"\"; ";
	if(transcript_type != "") fout<<"transcript_biotype \""<<transcript_type.c_str()<<"\"; ";
	fout<<"RPKM \""<<RPKM<<"\"; ";
	fout<<"cov \""<<coverage<<"\"; ";
	fout<<"empty_vertex_list \"";
        for(int k = 0; k < nfp.size(); k++)
        {
                fout<<"("<<nfp[k][0]<<","<<nfp[k][1]<<")";
                if(k != nfp.size() -1) fout<<".";
        }
        fout<<"\"; ";
	...
	fout<<"score_list \"";
	for(int k = 0; k < sl.size(); k++) fout<<sl[k]<<"-";
	fout<<"\"; ";
		
	fout<<"clist \"";
	set<int>::iterator itr;
	for(itr = clist.begin(); itr != clist.end(); itr++) fout<<*itr<<"-";
	fout<<"\";"<<endl;
