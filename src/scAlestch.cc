...
{
	printf("total number of transcripts: %d\n", vref.size());
...
int scAletsch::gene_pair1(const string &file)
{
	...
	printf("one-one: # total = %d, # single-exon gene pairs = %d, # multi-exon gene pairs = %d\n", sgp.size()+mgp.size(), sgp.size(), mgp.size());
	...
	printf("one-multiple: # total = %d, # single-exon gene pairs = %d, # multi-exon gene pairs = %d\n", sgp1.size()+mgp1.size(), sgp1.size(), mgp1.size());
...
int scAletsch::gene_pair_mb(const string &file)
{
	...
	printf("# multi-exon gene pairs = %d\n", mgp.size());
	...
	printf("# total shared boundaries = %d, # total shared junctions = %d, # total shared intron-chains = %d\n", shared_b.size(), shared_j.size(), shared_ic.size());
	return 0;
}
...
{
	printf("path %ld: # nodes = %ld, (", idx, path.size());
...
int assign_gid(genome1 &gm, vector< set<int> > &subgraphs)
{
	...
	printf("# graphs = %d, isolated node/subgraph = %d\n", subgraphs.size(), iso_cnt);
...
int allow_path_one_junction(genome1 &gm, vector< vector<int> > &paths)
{
	...
	printf("# paths consists of > 2 transcripts = %d\n", new_paths.size());
	...
	printf("# paths allow n junction in gap = %d\n", new_paths.size());
	paths = new_paths;
        new_paths.clear();
...
// add cnum to clist if cell has iny overlap j
int build_clist_j(genome1 &gm, genome1 &gm3)
{
	map<string, set<int>> &jtocnum = gm3.jtocnum;
		...
		map<int, int> &cur_cjcount = gm.transcripts[k].cjcount;
...
int get_coverage_em(genome1 &gm, genome1 &gm2)
{
	...
	for(int i = 0; i < gm2.transcripts.size(); i++)
	{
		...
		for(int j = 0; j < gm.transcripts.size(); j++)
		{
			...
			set<int> xpath(x.p.begin(), x.p.end());
...
int write_individual(genome1 &gm, const int &cnum, const string &prefix)
{
...
int store_features(genome1 &meta_gm, genome1 &gm, vector<vector<double>> &features)
{
	...
	for(int i = 0; i < meta_gm.transcripts.size(); i++)
		...
		// 1st feature: # junctions
		cur_feature.push_back(exons.size() - 1);

		// other features: score for each junction
		for(int i = 0; i < exons.size() - 1; i++)
		{
                	string h = curt.seqname;
                	h.append(tostring(exons[i].second));
		...
		// support coverage for each junction
		for(int i = 0; i < exons.size() - 1; i++)
                {
                        string h = curt.seqname;
                        h.append(tostring(exons[i].second));
		...
		// cell id, the number of junctions in this cell; 
...
int write_features(genome1 &gm, vector<vector<double>> &features, const string &prefix)
{
...
int addback_all_individuals(genome1& gm1, genome1& gm2)
{
	for(int i = 0; i < gm2.transcripts.size(); i++)
        {
                transcript &t = gm2.transcripts[i];
                if( t.exons.size() > 1)
                {
...
int scAletsch::link_merge(const string &prefix)
{
	...
	*/
	...
	//printf("# transcripts from individual: %ld", gm2.transcripts.size());	
	// add back all individuals to the scAletschd set
	//addback_all_individuals(gm1, gm3);
	//printf("# transcripts after adding back all individuals: %ld", gm1.transcripts.size());
	...
	write_features(gm, features, prefix);
	...
	*/
	write_individual(gm, cnum, prefix);
